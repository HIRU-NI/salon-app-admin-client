...
export const getPage = createAsyncThunk(
  ...
  async (params, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await clientService.getPage(token, params.page, params.sortby);
    } catch (error) {
      const message = error.response.data || error.message || error.toString();
      return thunkAPI.rejectWithValue(message);
    }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getPage.fulfilled, (state, action) => {
        state.isSuccess = true;
        state.isLoading = false;
        state.currentPageClients = action.payload.clients;
...
export const { reset, resetPage } = clientSlice.actions;
